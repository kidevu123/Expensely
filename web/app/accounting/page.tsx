...
export default function Accounting(){
  ...
  function exportCsvForOrg(orgLabel:string){
    ...
    items.forEach(e=>{
      const url = e.file_id? `${API}/files/${e.file_id}`:'';
      const row = [
    ...
    <div draggable onDragStart={onDragStart} className="rounded-xl border border-slate-200 p-2 text-sm flex items-center justify-between bg-white">
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          {e.file_id && setPreviewUrl && (
            <button className="text-xs text-blue-600 underline" onClick={()=>setPreviewUrl(`${API}/files/${e.file_id}`)}>view receipt</button>
          )}
        </div>
      </div>
      <div className="flex gap-2">
